...
let
  ...
  # For dev usage
  start-all-servers-in-memory = pkgs.writeShellScriptBin "plutus-pab-all-servers-in-memory" ''
    export FRONTEND_URL=https://localhost:8009
    export WEBGHC_URL=http://localhost:8080
    # There might be local modifications so only copy when missing
    ! test -f ./plutus-pab.yaml && cp ../plutus-pab/plutus-pab.yaml.sample plutus-pab.yaml
...
in
{
  inherit client demo-scripts server-invoker generated-purescript generate-purescript migrate start-backend start-all-servers start-all-servers-in-memory mkConf pab-exes;
}
