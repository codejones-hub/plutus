...
toInstanceState ContractInstanceStateInternal{cisiSuspState=SuspendedContract{scResumableResult}, cisiEvents, cisiHandlersHistory} =
    ContractInstanceState
        { instContractState = scResumableResult
...
addEventInstanceState event ContractInstanceStateInternal{cisiSuspState, cisiEvents, cisiHandlersHistory} =
    case Contract.Types.runStep cisiSuspState event of
        Nothing -> Nothing
        Just newState ->
            let SuspendedContract{scResumableResult=ResumableResult{wcsRequests=Requests rq}} = cisiSuspState in
