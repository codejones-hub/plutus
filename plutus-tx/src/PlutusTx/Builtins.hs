...
decodeUtf8 x = fromBuiltin (BI.decodeUtf8 (toBuiltin x))
...
appendString x y = fromBuiltin (BI.appendString (toBuiltin x) (toBuiltin y))

{-# INLINABLE emptyString #-}
-- | An empty 'String'.
emptyString :: BuiltinString
emptyString = fromBuiltin BI.emptyString

{-# INLINABLE charToString #-}
-- | Turn a 'Char' into a 'String'.
charToString :: Char -> BuiltinString
charToString c = fromBuiltin (BI.charToString (toBuiltin c))

{-# INLINABLE equalsString #-}
-- | Check if two strings are equal
equalsString :: BuiltinString -> BuiltinString -> Bool
equalsString x y = fromBuiltin (BI.equalsString (toBuiltin x) (toBuiltin y))

{-# INLINABLE trace #-}
-- | Logs the given 'String' to the evaluation log.
trace :: BuiltinString -> ()
trace s = fromBuiltin (BI.trace (toBuiltin s))

{-# INLINABLE encodeUtf8 #-}
-- | Convert a String into a ByteString.
encodeUtf8 :: BuiltinString -> ByteString
encodeUtf8 s = fromBuiltin (BI.encodeUtf8 (toBuiltin s))
