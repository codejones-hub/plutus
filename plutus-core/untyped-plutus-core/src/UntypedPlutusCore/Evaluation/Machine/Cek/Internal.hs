...
data CekValue uni fun =
  ...
  | VGLamAbs Word (Term Name uni fun ()) (CekValEnv uni fun)
...
type CekGValEnv uni fun s = STArray.STArray s Word (CekValue uni fun)
...
dischargeCekValue = \case
    ...
    VGLamAbs  name body env -> dischargeCekValEnv env $ GLamAbs () name body
...
lookupVarName n varEnv =
    case lookupName n varEnv of
        Nothing  -> throwingWithCause _MachineError OpenTermEvaluatedMachineError $ Just (Var () n)
        Just val -> pure val
...
enterComputeCek = computeCek (toWordArray 0) where
    ...
    computeCek !unbudgetedSteps ctx _ (GVar _ name) = do
        !unbudgetedSteps' <- stepAndMaybeSpend BVar unbudgetedSteps
        val <- CekCarryingM $ STArray.readArray ?cekGEnv name
        returnCek unbudgetedSteps' ctx val
    ...
    computeCek !unbudgetedSteps ctx env (GLamAbs _ name body) = do
        !unbudgetedSteps' <- stepAndMaybeSpend BLamAbs unbudgetedSteps
        returnCek unbudgetedSteps' ctx (VGLamAbs name body env)
    ...
    applyEvaluate !unbudgetedSteps ctx (VGLamAbs name body env) arg = do
