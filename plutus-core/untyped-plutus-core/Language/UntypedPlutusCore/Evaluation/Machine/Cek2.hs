...
module Language.UntypedPlutusCore.Evaluation.Machine.Cek2
    ( CekValue(..)
...
type TermWithMem uni fun = Term NamedDeBruijn uni fun ExMemory
...
data CekValue uni fun =
  ...
  | VLamAbs ExMemory NamedDeBruijn (TermWithMem uni fun) (CekValEnv uni fun)
  ...
  where
   go !lvl = \case
    var@(Var _ name) -> let n = name^.unique.coerced
                       in if n < lvl
                          then var
                          else dischargeCekValue $ valEnv `Seq.index` (n - lvl)
...
computeCek
    ...
    => Context uni fun -> CekValEnv uni fun -> TermWithMem uni fun -> CekM uni fun (Term NamedDeBruijn uni fun ())
-- s ; ρ ▻ {L A}  ↦ s , {_ A} ; ρ ▻ L
...
throwingDischarged
    :: ( MonadError (ErrorWithCause e (Term NamedDeBruijn uni fun ())) m
       , Closed uni, uni `Everywhere` ExMemoryUsage
       )
...
returnCek
    ...
    => Context uni fun -> CekValue uni fun -> CekM uni fun (Term NamedDeBruijn uni fun ())
...
applyEvaluate
    ...
    -> CekM uni fun (Term NamedDeBruijn uni fun ())
...
runCek
    ...
    -> (Either (CekEvaluationException uni fun) (Term NamedDeBruijn uni fun ()), CekExBudgetState fun)
