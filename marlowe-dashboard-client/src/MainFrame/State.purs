        ...
        NewObservableState rawJson -> do
          ...
          for_ mPlayState \playState -> do
            ...
            if (plutusAppId == walletCompanionAppId) then case runExcept $ decodeJSON $ unwrap rawJson of
              ...
              Right companionState ->
                let
                  lastObservedState = view (_walletDetails <<< _companionAppLastObservedState) playState
                in
                  -- for reasons yet to be uncovered the websocket sends too many of these messages, saying the state
                  -- has changed when it hasn't - so until that's fixed we filter here
                  when (companionState /= lastObservedState) $ handleAction $ PlayAction $ Play.UpdateRunningContracts companionState
            else
              ...
              if (plutusAppId == marloweAppId) then
                ...
                pure unit
              else -- otherwise this should be one of the wallet's WalletFollowerApps
...
handleAction (EnterPlayState walletLibrary walletDetails) = do
  ...
  case ajaxFollowerApps of
    ...
    Right followerApps -> do
      --let testingFollowerApps = followerApps <> dummyContracts currentSlot
