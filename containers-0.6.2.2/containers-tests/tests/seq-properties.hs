...
import           Data.Sequence.Internal               (Digit (..), Elem (..), FingerTree (..), Node (..), Seq (Seq),
                                                       Sized (..), deep, node2, node3)
...
import           Data.Foldable                        (Foldable (fold, foldMap, foldl, foldl1, foldr, foldr1), all,
                                                       foldl', foldr', sum, toList)
import           Data.Function                        (on)
import           Data.Functor                         ((<$), (<$>))
import           Data.Maybe
import           Data.Monoid                          (All (..), Dual (..), Endo (..), Monoid (..))
#if MIN_VERSION_base(4,9,0)
import           Data.Semigroup                       (stimes, stimesMonoid)
#endif
import qualified Data.List
import           Data.Traversable                     (Traversable (traverse), sequenceA)
import           Prelude                              hiding (all, drop, filter, foldl, foldl1, foldr, foldr1, length,
                                                       lookup, null, replicate, reverse, scanl, scanl1, scanr, scanr1,
                                                       splitAt, sum, take, zip, zip3, zipWith, zipWith3)
